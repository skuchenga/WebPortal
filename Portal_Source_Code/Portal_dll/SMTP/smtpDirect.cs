...
namespace HFCPortal.SMTP
{
    ...
    class smtpDirect
    {
        ...
        public static bool Send(System.Net.Mail.MailMessage message)
        {
            ...
            string _To = message.To.ToList().ToString();
            ...
            if (message.CC != null)
            {
                Tos = message.CC.ToList().ToString().Split(new char[] { ';' });
                foreach (string To in Tos)
                {
                    Senddata(s, string.Format("RCPT TO: {0}\r\n", To));
                    if (!Check_Response(s, SMTPResponse.GENERIC_SUCCESS))
            ...
            Tos = message.To.ToList().ToString().Split(new char[] { ';' });
            Header.Append("To: ");
